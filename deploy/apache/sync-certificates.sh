...
CONFIG_FILE="${CONFIG_FILE:-/etc/apache2/sites-enabled/000-default.conf}"
...
echo "apache-sync -- Using certificate ${EFFECTIVE_TLS_CERTIFICATE_FILE}"
echo "apache-sync -- Using key ${EFFECTIVE_TLS_CERTIFICATE_KEY}"

sed -i "s#^\\s*SSLCertificateFile .*#    SSLCertificateFile ${EFFECTIVE_TLS_CERTIFICATE_FILE}#" "${CONFIG_FILE}"
sed -i "s#^\\s*SSLCertificateKeyFile .*#    SSLCertificateKeyFile ${EFFECTIVE_TLS_CERTIFICATE_KEY}#" "${CONFIG_FILE}"
