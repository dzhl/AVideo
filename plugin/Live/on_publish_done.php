...
_error_log("NGINX ON Publish Done  POST: " . json_encode($_POST), AVideoLog::$DEBUG, true);
_error_log("NGINX ON Publish Done  GET: " . json_encode($_GET), AVideoLog::$DEBUG, true);
_error_log("NGINX ON Publish Done  php://input" . file_get_contents("php://input"), AVideoLog::$DEBUG, true);
...
_error_log("NGINX ON Publish Done  parse_url: " . json_encode($parts), AVideoLog::$DEBUG, true);
_error_log("NGINX ON Publish Done  parse_str: " . json_encode($_GET), AVideoLog::$DEBUG, true);
...
if ($_POST['name'] == 'live') {
    _error_log("NGINX ON Publish Done  wrong name {$_POST['p']}", AVideoLog::$WARNING, true);
    // fix name for streamlab
    $pParts = explode("/", $_POST['p']);
    if (!empty($pParts[1])) {
        _error_log("NGINX ON Publish Done  like key fixed", AVideoLog::$DEBUG, true);
        $_POST['name'] = $pParts[1];
    }
}
...
_error_log("NGINX ON Publish Done finishFromTransmitionHistoryId {$_POST['name']} id={$row['id']} key={$row['key']} live_servers_id={$row['live_servers_id']} insert_row={$insert_row}", AVideoLog::$DEBUG, true);
...
if(empty($row)){
    $sql = $getLatestSQL;
    _error_log("NGINX ON Publish Done error LiveTransmitionHistory::getLatest({$_POST['name']}, $live_servers_id, true); time=".time().' '.json_encode(array($whatIFound, $sql)), AVideoLog::$WARNING, true);
    $row = LiveTransmitionHistory::getLatest($_POST['name'], $live_servers_id);
}
if(!empty($row)){
    _error_log("NGINX ON Publish Done success ({$row['id']}, {$row['users_id']}, {$row['key']}, {$row['live_servers_id']})", AVideoLog::$DEBUG, true);
    AVideoPlugin::on_publish_done($row['id'], $row['users_id'], $row['key'], $row['live_servers_id']);
}else{
    _error_log("NGINX ON Publish Done error, nothing found LiveTransmitionHistory::getLatest({$_POST['name']}, $live_servers_id, true); ", AVideoLog::$WARNING, true);
}
