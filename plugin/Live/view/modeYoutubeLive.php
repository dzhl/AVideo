...
if (!empty($_REQUEST['live_schedule'])) {
    ...
    if (empty($scheduleLiveServerId)) {
        // Last resort: look up the live_servers_id from the live_transmitions table
        // Schedule URLs generated by the gallery use live_servers_id=0, so we need the DB value.
        $ltData = LiveTransmition::getFromKey($ls->getKey());
        if (!empty($ltData) && !empty($ltData['live_servers_id'])) {
            $scheduleLiveServerId = intval($ltData['live_servers_id']);
        }
    }
